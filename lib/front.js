...
export default class Front {
  ...
  init() {
    ...
    // bind button for loading remote RMLIO rules to corresponding function
    $('#rmlio-load-matey').on('click', () => {
      const url = prompt("Enter RMLIO rules URL");

      if (url !== null) {
        this.matey.loadRemoteRmlIo(url);
      }
    });

    // bind button for loading remote RMLKGC rules to corresponding function
    $('#rmlkgc-load-matey').on('click', () => {
      const url = prompt("Enter RMLKGC rules URL");

      if (url !== null) {
        this.matey.loadRemoteRmlKgc(url);
      }
    });
    ...
    $('#rmlio-dl-matey').on('click', () => {
      downloadString(this.matey.editorManager.getRMLIO(), 'text', 'rmlio.ttl');
    });

    $('#rmlkgc-dl-matey').on('click', () => {
      downloadString(this.matey.editorManager.getRMLKGC(), 'text', 'rmlkgc.ttl');
    });
  ...
  updateLayout(layout) {
    ...
    const rmlIoDiv = $('#div-rmlio-matey');
    const rmlKgcDiv = $('#div-rmlkgc-matey');
    ...
    switch (layout) {
      case '2x2':
        ...
        rmlIoDiv.attr('class', 'col-md-6');
        rmlKgcDiv.attr('class', 'col-md-6');
      ...
      default:
        ...
        rmlIoDiv.attr('class', 'col-md-4');
        rmlKgcDiv.attr('class', 'col-md-4');
  ...
  /**
   * Fills the HTML select element and configures the button to select the mode.
   * @param {Array} modes - modes for which buttons must be made
   */
  loadModes(modes) {
    const $el = $('#modes-matey');

    modes.forEach((mode) => {
      const $option = $('<option>', { value: mode, text: mode });
      $el.append($option);
    });

    $el.on('change', () => {
      const selectedMode = $el.val();
      this.matey.setMode(selectedMode);
    });
  }

  /**
   * Adapts the front to work in given mode
   * 
   * @param {String} mode - mode to be set
   */
  setMode(mode) {
    const $el = $('#modes-matey');
    const modes = $el.find('option').map((i, el) => $(el).val()).get();
    ...
    switch (mode) {
      case 'YARRRML':
      default:
        $('#div-rmlio-matey').hide();
        $('#div-rmlkgc-matey').hide();
        $('#div-yarrrml-matey').show();
        break;
      case 'RMLIO':
        $('#div-yarrrml-matey').hide();
        $('#div-rmlkgc-matey').hide();
        $('#div-rmlio-matey').show();
        break;
      case 'RMLKGC':
        $('#div-yarrrml-matey').hide();
        $('#div-rmlio-matey').hide();
        $('#div-rmlkgc-matey').show();
        break;
    }
  }
  ...
  doAlert(message, type = 'primary', timeout = 2000) {
    ...
    const htmlTmpl = compile(alertHtmlSource);
    const html = htmlTmpl({
      type: type,
      message: message
    });
