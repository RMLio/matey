...
// utilities
const {prettifyRDF, loadRemote, downloadString, doAlert} = require("./util/util");
...
class Matey {
  ...
  createDataEditor(dataPart, index, selectValue = null) {
    ...
    $dropdownA.on('click', e => {
      this.mateyFront.$inputButtonDiv.html(inputButton);
      this.mateyFront.updateDeleteButton(index);
  ...
  deleteDataEditor(index) {
    ...
    if (this.dataEditors.length > 0) {
      ...
      this.mateyFront.updateDeleteButton(null);
    }
  }
