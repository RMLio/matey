...
class Matey {
  ...
  init(id, config) {
    ...
    document.getElementById("rml-btn-matey").onclick = () => {
      this.toRML().catch(err => {
        $logger.error('rml_generation_error', err);
        doAlert('Could not generate RML rules.', 'danger', 3000);
      });
    };

    document.getElementById("ld-btn-matey").onclick = () => {
      this.runMappingRemote().catch(err => {
        $logger.error('ld_generation_error', err);
        doAlert('Could not generate Linked Data.', 'danger', 3000);
      })
    };
   ...
   * @returns {Promise<void>} empty promise that fetching has been executed
   */
   async loadRemoteDataSource(url, dataPath) {
    try {
      let dataValue = await loadRemote(url);
      this.createAndOpenDataEditor(dataPath, dataValue);
    } catch (e) {
      $logger.error('data_loading_error', e);
      doAlert('Could not load remote data source.', 'danger', 5000);
    }
  }
   ...
   * @returns {Promise<void>} empty promise that fetching has been executed
   */
  async loadRemoteYarrrml(url) {
    try {
      let rules = await loadRemote(url);
      this.editor.setValue(rules);
    } catch (e) {
      $logger.error('yarrrml_loading_error', e);
      doAlert('Could not load remote YARRRML rules.', 'danger', 5000);
    }
  }
        ...
        try {
          ...
          writer.end(async (error, result) => {
            ...
            try {
              ...
              $logger.error('prettify_failed', e)
            }
