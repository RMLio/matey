...
class Matey {
          ...
          fetch(this.rmlMapperUrl, {
            ...
            if (typeof data.output === 'string' || data.output instanceof String) {
              ...
              parser.parse(data.output, (err, quad) => {
                if (quad) {
                  ...
                  outWriter.end(async (err, outTtl) => {
                    ...
                    let outputEditor = this.createOutputEditor({path: 'output', type: 'text', value: outTtl}, 0);
              ...
              for (const [file, content] of Object.entries(data.output)) {
                ...
                let newIndex = this.outputEditors.length > 0 ? this.outputEditors[this.outputEditors.length - 1].index + 1 : 0;
                let outputEditor = this.createOutputEditor({path: file, type: 'text', value: content}, newIndex);
                this.outputEditors.push(outputEditor);
  ...
  loadExample(example, reset = false) {
    ...
    dataParts.forEach((dataPart, index) => {
      const editor = this.createDataEditor(dataPart, index, selectValue);
  ...
  createOutputEditor(dataPart, index, selectValue = null) {
    return this.createAbstractEditor(dataPart, index, selectValue, 'outputeditor', 'output-matey', 'dropdown-out-chooser', true);
  }
