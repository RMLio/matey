...
class Matey {
   ...
   */
   async loadRemoteDataSource(url, dataPath) {
    try {
      ...
      $logger.error('data_loading_error', e, 5000);
      doAlert('Could not load remote data source.', 'danger');
    }
  }
  ...
  async loadRemoteYARRRML(url) {
    try {
      ...
      $logger.error('yarrrml_loading_error', e, 5000);
      doAlert('Could not load remote YARRRML rules.', 'danger');
    }
  }
