...
class Matey {
  ...
  /**
   * Fetch a remote data source, and use it to create a new data editor. Returns a promise that the data source is fetched.
  ...
  /**
   * Fetch YARRRML rules from remote source and set YARRRML editor's value to them. Returns a promise that the rules are fetched.
   * @param {String} url of remote rules
   */
  loadRemoteYARRRML(url) {
    return new Promise((resolve, reject) => {
      loadRemote(url, rules => {
...
/**
 * Fetch a remote source with given URL asynchronously and execute callback with response if request is successful
 * @param url of remote source
 * @param callback function that gets executed if request successful
 */
  ...
  xhr.onreadystatechange = () => {
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        ...
        doAlert('An error has occured while loading the source.', 'danger');
