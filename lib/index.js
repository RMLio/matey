  ...
  init(id, config = {rml_mapper_uri: "http://localhost:4000/execute"}) {
    ...
    $('#data-load-matey').on('click', () => {
      ...
      if (url !== null && dataPath !== null) {
        // extract file type out of dataPath
        let extension = dataPath.split('.').pop();
        let type = extension === 'json' ? 'json' : 'text';
        ...
        xhr.onreadystatechange = () => {
          if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
            let dataValue = xhr.responseText;
...
/**
 * Makes text with RDF quads prettier
 * @param {String} rdf unprettified RDF
 * @param {Function} callback function that gets called when stream has finished writing all quads
