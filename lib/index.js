...
// front
const MateyFront = require('./matey-front');
...
class Matey {
  ...
  constructor() {
    ...
    // matey frond
    this.mateyFront = new MateyFront(this, this.persister, this.logger);
  }
  ...
  init(id, config) {
    ...
    this.mateyFront.loadExamples('examples-matey', this.mateyExamples);
    let stored = this.persister.get('latestExample');

    if (stored) {
      this.mateyFront.doAlert('We found a previous edited state in our LocalStorage you used to successfully generate RDF! I hope you don\'t mind we loaded that one for you ;).', 'info', 10000);
  ...
  loadRemoteDataSource(url, dataPath) {
    return loadRemote(url)
      ...
      .catch(e => {
        ...
        this.mateyFront.doAlert('Could not load remote data source.', 'danger', 5000);
      })
  }
  ...
  loadRemoteYarrrml(url) {
    return loadRemote(url)
      ...
      .catch(e => {
        ...
        this.mateyFront.doAlert('Could not load remote YARRRML rules.', 'danger', 5000);
      })
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        writer.end(async (error, result) => {
          ...
          this.mateyFront.doAlert('RML mapping file updated!', 'success');
        ...
        this.mateyFront.doAlert('Could not generate RML rules.', 'danger', 3000);
      }
    });
  }
  ...
  runMappingRemote() {
    return new Promise((resolve, reject) => {
      ...
      try {
              ...
              parser.parse(data.output, (err, quad) => {
                if (quad) {
                  ...
                  outWriter.end(async (err, outTtl) => {
                    ...
                    this.mateyFront.doAlert('Output updated!', 'success');
        ...
        this.mateyFront.doAlert('Could not generate Linked Data.', 'danger', 3000);
      }
    });
  };
   ...
   */
  destroyEditors() {
    this.mateyFront.destroyEditors();
    this.dataEditors = [];
  }
  ...
  loadExample(example, reset = false) {
    ...
    if (reset) {
      this.mateyFront.doAlert(`Reloaded the "${example.label}" example`, 'success');
    }
  }
  ...
  createDataEditor(dataPart, index, selectValue = null) {
    ...
    return {
      ...this.mateyFront.createDataEditor(dataPart.path, index, value),
  ...
  deleteDataEditor(index) {
    ...
    if (this.dataEditors.length > 0) {
      ...
      this.mateyFront.deleteDataEditor();
    }
  }
